<script setup>
  ...
  import ElbowTeeRegistryManage from './ElbowTeeRegistryManage.vue';
  ...
  function duan(pipeids) {
    let pids = JSON.parse(pipeids);
    if (!Array.isArray(pids)) {
      pids = [pids];
    }
    const pipeNames = pids.map((item) => {
      const pipe = pipes.value.find((pipe) => pipe.id == item);
      return pipe ? pipe.pipeName : ''; // Return an empty string if pipe is not found
    });
    return pipeNames.join(', ');
  }
