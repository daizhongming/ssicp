<script setup>
  ...
  async function dadio(i, id) {
    ...
    if (i == 1) {
      ...
      notpid.value = devicedto.deviceValve.pipeInlet;
    } else {
      //进口
      inletOrexit.value = 'inlet';
      notpid.value = devicedto.deviceValve.pipeExit;
    }
  ...
  async function radiook(pid) {
    ...
    if (inletOrexit.value === 'exit') {
      //出口
      console.log('出口');
      devicedto.deviceValve.pipeExit = pid;
    } else if (inletOrexit.value === 'inlet') {
      //进口
      console.log('进口');
      devicedto.deviceValve.pipeInlet = pid;
    } else {
      return;
    }
  }
...
<template>
  <FormCard title="阀门基本信息">
    <el-form label-width="80px">
      <el-row :gutter="16">
        <el-col :span="12">
          <el-form-item label="阀门编号" required>
            <el-input
              minlength="3"
              maxlength="10"
              show-word-limit
        ...
        <el-col :span="12">
          <el-form-item label="阀门名称" required>
            <el-input
              minlength="3"
              maxlength="10"
              show-word-limit
        ...
        <el-col :span="12">
          <el-form-item label="阀门类型" required>
            <el-select
              clearable
            ...
            >
              <el-option
                v-for="yali in dictStore().pipeline_famen_type().all"
        ...
        <el-col :span="12">
          <el-form-item label="工作年限">
            <el-input
              v-model="devicedto.deviceValve.workYears"
              placeholder="请输入"
              minlength="3"
              maxlength="10"
              show-word-limit
        ...
        <el-col :span="12">
          <el-form-item label="投用日期">
            <el-date-picker
              clearable
              v-model="devicedto.deviceValve.useTime"
              type="date"
              label="请选择日期"
              placeholder="请选择日期"
            />
        ...
        <el-col :span="12">
          <el-form-item label="使用年限">
            <el-input
              v-model="devicedto.deviceValve.usageDuration"
              placeholder="请输入"
              minlength="3"
              maxlength="10"
              show-word-limit
        ...
        <el-col :span="12">
          <el-form-item label="检查周期">
            <el-input
              v-model="devicedto.deviceValve.checkPeriod"
              placeholder="请输入"
              minlength="3"
              maxlength="10"
              show-word-limit
        ...
        <el-col :span="12">
          <el-form-item label="质检日期">
            <el-date-picker
              clearable
              v-model="devicedto.deviceValve.qcDate"
              type="date"
              label="请选择日期"
              placeholder="请选择日期"
            />
      ...
      <el-row :gutter="16">
        <el-col :span="12">
          <el-form-item label="管段入口" required>
            <el-input
              readonly="readonly"
              :value="inlet(devicedto.deviceValve.pipeInlet)"
              v-model="devicedto.deviceValve.pipeInlet"
              clearable
              @click="dadio(0, devicedto.deviceValve.pipeInlet)"
              placeholder="必填项"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="管段出口" required>
            <el-input
              readonly="readonly"
              :value="inlet(devicedto.deviceValve.pipeExit)"
              v-model="devicedto.deviceValve.pipeExit"
              clearable
              @click="dadio(1, devicedto.deviceValve.pipeExit)"
              placeholder="必填项"
            />
          </el-form-item>
        </el-col>
  ...
  <FormCard title="监测指标报警设置">
    <ParametersettingsManage
      ...
      :dictData="dictStore().valve_index().all"
    />
  </FormCard>
